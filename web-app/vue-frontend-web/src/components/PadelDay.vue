        ...
        <v-row>
          ...
          <v-col xs="2">
            <v-btn
              ...
              :disabled="
                (hora.reserva && hora.reserva.isBooked) ||
                currentDateIsNotCorrect
              "
              >Reservar hora</v-btn
            >
            <div v-if="hora.reserva != null">
              <p
                class="caption grey--text"
                v-if="hora.reserva.isBooked === true"
              >
                Hora reservada.
              </p>
            </div>
...
export default {
  ...
  mounted() {
    ...
    PadelService.getAllBookingsInThisDay(this.date).then(
      (response) => {
  ...
  methods: {
    atras() {
      this.$router.push("/padel");
    },
    handleReservarHora(hora) {
      PadelService.createBooking(this.date, hora, this.currentUser.id).then(
        (response) => {
          console.log(response);
          this.content = response.data.message;
          swal("¡Padel reservado!", this.content, "success");
        ...
        (error) => {
          ...
          swal("¡Error al reservar pádel!", error.response.data.message.toString(), "error");
        }
      );
    },
  },
